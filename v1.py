...
# 添加异常值标记列的选择框
anomaly_column = st.selectbox("Select Anomaly Column", ["isolation_forest_anamoly", "lof_anamoly", "svm_anamoly", "knn_anamoly", "final_anomaly_flag"])
...
if uploaded_files:
    ...
    fig = px.scatter_mapbox(all_data, 
                      ...
                      color_discrete_map={'red': 'red', 'blue': 'blue'},  # 自定义颜色映射
                      labels={'marker_color': 'Anomaly Condition'})  # 自定义颜色映射标签
    ...
    if uploaded_geojson:
        ...
        # 添加Railway line sections的轨迹数据到地图图层
        for section in railway_sections_data['features']:
            ...
            line = go.Scattermapbox(
                ...
                name=f"Railway Line {section['properties']['id']}",
                hovertext=f"Railway Line {section['properties']['id']}"  # 添加悬停文本
            )
            fig.add_trace(line)
